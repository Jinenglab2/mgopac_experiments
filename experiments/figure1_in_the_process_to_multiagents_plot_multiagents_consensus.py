...
for Learner in LEARNERS:
    ...
    for run in range(RUNS):
        for problem in PROBLEMS:
            ...
            for episode in range(EPISODES):
                ...
                # ----- record the j_mu for each agent ###multi-agent!!!----- #
                j_mu1 = []
                for wi in w:
                    j_mu1.append(J_MU(wi))
                ...
                collector1.collect(Learner.__name__, j_mu1)
    ...
    # plot for multi agents !##
    data1 = collector1.getStats(name)
    ...
    for i in range(min(NUM_ACTORS, 4)):
        ...
        # + ' $\lambda=$' + str(lmda)
        plot1(ax, [data1[0][:, i], data1[1][:, i], data1[2]], label=name + str(2 * i + 1),
